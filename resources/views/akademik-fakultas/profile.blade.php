...
<x-layout :authUser='$authUser'>
    <x-slot:title>
        Akademik Fakultas | Profile
    </x-slot:title>

    <h1 class="mx-auto text-center font-bold mb-2">Pengaturan Akun</h1>

    <form action="{{ route('update-profile-akademik-fakultas', $authUser->id) }}" enctype="multipart/form-data"
        method="POST" class="md:grid flex  md:grid-cols-2 gap-4 justify-center mx-auto flex-col  w-full">
        @csrf
        @method('PUT')
        <div class="mb-6">
        ...
        <div class="mb-6 hidden ">
            <label for="jurusan" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jurusan Akademik
            </label>
            <select type="text" disabled
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                required>
                <option value="">Pilih Akademik Jurusan</option>
                @foreach ($daftarJurusan as $jurusan)
                    <option {{ $authUser->jurusan_id == $jurusan->id ? 'selected' : '' }} value="{{ $jurusan->id }}">
                        {{ $jurusan->name }}</option>
                @endforeach
                <input type="text" hidden name="jurusan" value="{{ $authUser->jurusan_id }}">
            </select>
        </div>
        {{-- <div class="mb-6">
        ...
        <div class="col-span-2 flex w-full justify-center gap-2 items-center mx-auto mt-0">
            ...
            <a class="bg-rose-600 p-2 text-white font-medium hover:bg-rose-700  rounded-lg flex {{ $edit ? 'hidden' : '' }}"
                href="/akademik-fakultas/profile/reset-password">
