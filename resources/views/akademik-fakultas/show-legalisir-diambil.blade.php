...
<x-layout :authUser='$authUser'>
    <x-slot:title>
        Akademik Fakultas | Detail Pengajuan Legalisir
    </x-slot:title>
    <x-slot:script>
        <script>
    ...
    @if ($surat->status == 'diproses' || $surat->status == 'dikirim' || $surat->status == 'selesai')
        <a href="{{ route('print-surat-akademik-fakultas', $surat->id) }}"
    ...
    @endif
    <form id="approval-form" action="{{ route('setujui-surat-akademik-fakultas', $surat->id) }}" method="POST"
    ...
    <div id="confirmation-modal" class="fixed inset-0 flex items-center justify-center  hidden">
        <div class="bg-white rounded-lg shadow-lg p-6 w-96">
            ...
            <p class="mt-4">Apakah Anda yakin ingin menyetujui dan mengubah status ke selesai (Ambil di tempat) untuk
                pengajuan
                legalisir ini?</p>
