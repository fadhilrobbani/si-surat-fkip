...
<x-layout :authUser='$authUser'>
                            ...
                            <tr class="border-b border-gray-200 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4">
                                    {{-- <a class="text-blue-700 underline"
                                        href="{{ route('show-file-wd3', ['surat' => $surat->id, 'filename' => basename($value)]) }}">Lihat</a> --}}
                                    {{-- <a class="text-blue-700 underline"
    ...
    {{-- @if ($surat->current_user_id == auth()->user()->id && $surat->status == 'diproses')
            <div class="flex flex-col sm:flex-row">
                ...
                <form action="{{ route('setujui-surat-wd3', $surat->id) }}" method="POST">
                    @csrf
                    @method('put')
                    <button type="button"
                ...
                <a href="{{ route('confirm-tolak-surat-wd3', $surat->id) }}">
    ...
    @if (
        ...
        <form action="{{ route('setujui-surat-wd3', $surat->id) }}" method="POST"
                    ...
                    <div class="flex flex-col justify-center items-center max-w-[400px]">
                        <label for="ttd"
                            ...
                            Tangan Anda (Jika tidak sesuai/tidak muncul, Anda dapat mengubahnya di profil akun <a
                                class="underline text-blue-600" href="/wd3/profile">di sini</a>)
                        </label>
            ...
            <div class="flex mt-8 justify-between flex-col sm:flex-row ">
                <a href="{{ route('preview-surat-wd3', $surat->id) }}"><button type="button"
    ...
    @if (
        ...
        <form action="{{ route('setujui-surat-from-staff-wd3', $surat->id) }}" method="POST"
            class="bg-slate-100 rounded-lg w-full">
            @csrf
            @method('put')
            {{-- <div class=" flex flex-col gap-4 mt-10 items-center justify-center">
    ...
    @if (
        ...
        <form action="{{ route('setujui-surat-staff-dekan-wd3', $surat->id) }}" method="POST"
            class="bg-slate-100 rounded-lg w-full">
            @csrf
            @method('put')
            {{-- <div class=" flex flex-col gap-4 mt-10 items-center justify-center">
