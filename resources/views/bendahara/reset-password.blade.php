...
<x-layout :authUser='$authUser'>
    <x-slot:title>
        Bendahara | Atur Ulang Kata Sandi
    </x-slot:title>

    <h1 class="mx-auto text-center font-bold mb-2">Atur Ulang Kata Sandi</h1>

    <form action="{{ route('reset-password-bendahara', $authUser->id) }}" method="POST"
        class="flex gap-2 justify-center flex-col w-full max-w-md mx-auto">
        @csrf
        @method('PUT')
        <div class="mb-6">
            <label for="old-password" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kata sandi saat ini</label>
        ...
        <div class="mb-6">
            <label for="password_confirmation" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Konfirmasi kata sandi baru</label>
        ...
        <div class="flex w-full justify-center gap-2 items-center mx-auto mt-4">
            <a class="bg-pink-700 p-2 text-white font-medium hover:bg-pink-800 rounded-lg flex"
                href="/bendahara/profile">
