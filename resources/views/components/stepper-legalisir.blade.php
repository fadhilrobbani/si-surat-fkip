    ...
    if ($surat->data['pengiriman'] == 'dikirim') {
        $steps = [
            ...
            [
                ...
                'description' => 'Dokumen Legalisir telah diterima',
            ],
        ];
    } elseif ($surat->data['pengiriman'] == 'ambil') {
        $steps = [
            [
                'status' => 'menunggu_pembayaran',
                'label' => 'Menunggu Pembayaran',
            ...
            [
                'status' => 'selesai',
                'label' => 'Dokumen Diterima',
                'description' => 'Dokumen Legalisir telah siap. Silakan datang ke Akademik untuk mengambil',
            ],
        ];
    }
        ...
        @foreach ($steps as $index => $step)
            <li class="{{ $index < count($steps) - 1 ? 'mb-10' : '' }} ml-6">
                ...
                <p class="text-sm {{ $isCurrent ? 'text-blue-700 dark:text-white' : '' }}">
                    @if ($isError)
                        {{ $isRejected ? 'Ditolak' : 'Gagal diproses' }}
                    @elseif($isCompleted)
                        {{ $step['description'] }}
                    @else
