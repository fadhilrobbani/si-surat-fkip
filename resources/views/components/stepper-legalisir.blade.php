    ...
    if ($surat->data['pengiriman'] == 'dikirim') {
        $steps = [
            [
                ...
                'description' => 'Anda telah mengajukan legalisir',
            ],
            [
                'status' => 'diproses',
                'label' => 'Diproses Akademik',
                'description' => 'Pengajuan Legalisir telah diproses',
            ],
            [
                'status' => 'dikirim',
                'label' => 'Dikirim ke Alamat Tujuan',
                'description' => 'Dokumen Legalisir telah dikirim',
            ],
        ...
        $steps = [
            ...
            [
                'status' => 'selesai',
                'label' => 'Dokumen Siap Diambil',
                'description' => 'Dokumen Legalisir telah siap. Silakan datang ke Akademik',
            ],
        ];
    }
        ...
        @foreach ($steps as $index => $step)
            <li class="{{ $index < count($steps) - 1 ? 'mb-10' : '' }} ml-6">
                @php
                    ...
                    $isError =
                        ($isRejected && $index >= $currentStepIndex) || ($isExpired && $index >= $currentStepIndex);
                @endphp
                ...
                <p class="text-sm {{ $isCurrent ? 'text-blue-700 dark:text-white' : '' }}">
                    @if ($isError)
                        {{ $isExpired && $index === 0 ? 'Tidak Dikonfirmasi' : ($isRejected ? 'Ditolak' : 'Gagal diproses') }}
                    @elseif($isCompleted)
                        {{ $step['description'] }}
                    @else
