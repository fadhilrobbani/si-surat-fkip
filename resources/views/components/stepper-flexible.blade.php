    ...
    <ol class="relative mx-8 text-gray-500 border-l border-gray-200 dark:border-gray-700 dark:text-gray-400">
        @foreach ($roles as $role)
            ...
            @if ($roleModel)
                <li class="mb-10 ml-6">
                    @if (
                        ($surat->status == 'ditolak' || ($surat->expired_at && $surat->expired_at < Carbon\Carbon::now() && $surat->status === 'diproses')) &&
                            $loop->index == $lastKey)
                        <span
                        ...
                        <h3 class="font-medium leading-tight">{{ $roleModel->description }}</h3>
                        <p class="text-sm">
                            {{ $surat->status == 'ditolak' ? 'Ditolak' : 'Gagal Diteruskan' }}
                        </p>
                    @else
                        <span
                        ...
                        <h3 class="font-medium leading-tight">{{ $roleModel->description }}</h3>
                        <p class="text-sm">{{ $loop->first ? 'Berhasil Mengajukan' : 'Disetujui' }}</p>
                    @endif
                </li>
            @endif
        @endforeach
        @if ($surat->status == 'diproses' && $surat->current_user && $surat->current_user->role)
            <li class="mb-10 ml-6">
                <span
                ...
                <h3 class="font-medium leading-tight">{{ $surat->current_user->role->description }}</h3>
