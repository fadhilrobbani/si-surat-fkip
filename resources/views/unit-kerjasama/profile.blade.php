...
<x-layout :authUser='$authUser'>
    <x-slot:title>
        Unit Kerjasama | Profile
    </x-slot:title>

    <h1 class="mx-auto text-center font-bold mb-2">Pengaturan Akun</h1>

    <form action="{{ route('update-profile-unit-kerjasama', $authUser->id) }}" method="POST"
        ...
        <div class="mb-6">
            ...
            <input type="text" id="username" name="username" {{ $edit ? '' : 'disabled' }}
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                placeholder="Masukkan Username" value="{{ $authUser->username }}" required>
        </div>

  

        <div class="flex w-full justify-center gap-2 items-center mx-auto mt-0">
            ...
            <a class="bg-rose-600 p-2 text-white font-medium hover:bg-rose-700  rounded-lg flex {{ $edit ? 'hidden' : '' }}"
                href="/unit-kerjasama/profile/reset-password">

                <svg class="w-6 h-6 text-white dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                    fill="none" viewBox="0 0 20 20">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M10 11V6m0 8h.01M20 10a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                </svg>
