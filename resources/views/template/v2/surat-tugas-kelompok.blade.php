...
<body>
    ...
    <table class="border">
        <tr>
            ...
            <td style="width: 58%; white-space: nowrap;">Nama, NIP, Pangkat, dan Golongan</td>
            <td>Jabatan</td>
        </tr>
        @if (isset($surat->data['dosen']) && is_array($surat->data['dosen']))
            @foreach ($surat->data['dosen'] as $index => $dosen)
                @php
                    $num = $loop->iteration;
                    $nama = $dosen['namaDosen' . $num] ?? ($dosen['namaDosen'] ?? ($dosen['nama'] ?? ''));
                    $nip = $dosen['nipDosen' . $num] ?? ($dosen['nipDosen'] ?? ($dosen['nip'] ?? ''));
                    $pangkat = $dosen['pangkatDosen' . $num] ?? ($dosen['pangkatDosen'] ?? ($dosen['pangkat'] ?? ''));
                    $jabatan = $dosen['jabatanDosen' . $num] ?? ($dosen['jabatanDosen'] ?? ($dosen['jabatan'] ?? ''));
                @endphp
                <tr>
                    <td style="width: 25px; text-align: center; vertical-align: top;">{{ $loop->iteration }}</td>
    ...
    <p style="text-align: justify">Demikianlah surat tugas ini dibuat untuk dapat dilaksanakan dengan sebaik-baiknya,
        dan tidak mengganggu tugas pokok. Setelah selesai diwajibkan memberikan laporan kepada pimpinan Fakultas.
    </p>
    <br><br>
    <div>
        {{-- ttd wd --}}
